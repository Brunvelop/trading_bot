...
class SuperStrategyFutures(Strategy):
    ...
    def calculate_std_dev_and_avg_range(self, data, short_window=10 , long_window=200):
        bar_range = (data['High'] - data['Low']).abs() / data['Low'] * 100
        avg_range = bar_range.ewm(span=short_window).mean()
        std_dev = avg_range.rolling(window=long_window).std()
